...
use Facades\App\Services\Translate\AzureTranslateService;
...
class LanguageController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if ($request->file('flag') && $request->file('flag')->isValid()) {
                $image = $this->fileUpload($request->flag, config('filelocation.language.path'), null, null, 'webp', 80, $language->flag, $language->flag_driver);
                if ($image) {
                    $flagImage = $image['path'];
