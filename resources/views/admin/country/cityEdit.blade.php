...
@section('page_title', __('Edit City'))
@section('content')
            ...
            <div class="d-grid gap-3 gap-lg-5">
                <div class="card">
                    <div class="card-header d-flex align-items-center justify-content-between">
                        <h2 class="card-title h4 mt-2">@lang('Edit City')</h2>
                        <a href="{{ route('admin.stateCity',['country'=>$country,'state'=> $state]) }}" class="btn btn-sm btn-info">
                            <i class="bi-arrow-left"></i> @lang('Back')
                        </a>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('admin.city.update', ['country' => $country->id, 'state' => $state->id, 'city' => $city->id]) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('patch')
                                    ...
                                    <label class="row form-check form-switch mt-3" for="city_status">
                                        ...
                                        <span class="col-2 col-sm-3 text-end">
                                            ...
                                            <input class="form-check-input @error('status') is-invalid @enderror" type="checkbox"
                                                   name="status" id="cityStatus" value="1" {{ old('status', $city->status) == 1 ? 'checked' : '' }}>
