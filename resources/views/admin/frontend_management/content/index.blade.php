                                                    ...
                                                    @if($type == "textarea")
                                                        <div class="col-md-12 mb-4">
                                                            ...
                                                            <textarea
                                                                ...
                                                                name="{{ $name }}[{{ $language->id }}]">
                                                                {{ old($name.'.'.$language->id, isset($singleContentData[$language->id]) ? @$singleContentData[$language->id][0]->description->{$name} : '') }}
                                                            </textarea>
                                                            @error($name.'.'.$language->id)
                                                            <span class="invalid-feedback d-block">{{ $message }}</span>
                                                            @enderror
                ...
                <div class="table-responsive datatable-custom">
                    <table id="datatable"
                        ...
                        @foreach($multipleContentData as $key => $data)
                            @php
                                $media = is_string($data->content?->media) ? json_decode($data->content?->media) : $data->content?->media ?? [];
                                $firstValue = reset($media) ?? null;
                            @endphp

                            <tr>
                                <td>{{ $loop->index + 1 }}</td>
                                <td> {!!  collect($data->description)->first() ?? $firstValue !!}</td>
                                <td>
