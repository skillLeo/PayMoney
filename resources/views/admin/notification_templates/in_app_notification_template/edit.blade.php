...
@section('page_title', __('Edit in app notification template'))
@section('content')
    <div class="content container-fluid">
        <div class="page-header">
            <div class="row align-items-end">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title">@lang("Edit in app notification template")</h1>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb breadcrumb-no-gutter">
                            ...
                            <li class="breadcrumb-item active"
                                aria-current="page">@lang("Edit in app notification template")</li>
                        </ol>
                    </nav>
                </div>
            </div>
                ...
                <div class="d-grid gap-3 gap-lg-5">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">@lang("Edit in app notification template")</h4>
                        </div>
                        <div class="card-body">
                            ...
                            <div class="tab-content">
                                @foreach($languages as $key => $value)
                                    <div
                                        ...
                                        <form method="post"
                                              action="{{ route('admin.in.app.notification.template.update', [$template->id, $value->id]) }}" enctype="multipart/form-data"
                                              class="base-form">
                                            @csrf
                                            @method('put')
                                            <div
                                                class="my-3 section-title">{{trans('Mail & SMS in')}}  {{ __($value->name) }}
                                                ...
                                                <div class="col-md-6">
                                                    <label class=" row form-check form-switch"
                                                        ...
                                                        <span class="col-4 col-sm-3 text-end">
                                                                <input type='hidden' value='0' name='in_app_status'>
                                                                <input
                                                                    class="form-check-input @error('in_app_status') is-invalid @enderror"
                                                                    type="checkbox" name="in_app_status"
                                                                    id="status[{{ $value->id }}]"
                                                                    value="1" {{ old('in_app_status.'.$value->id, $templates[$key]->status['in_app'] ?? 0) == 1 ? 'checked' : ''}}>

                                                            </span>
                                                        @error('in_app_status.'.$value->id)
                                                        <span class="text-danger">{{ __($message) }}
                                                    </span>
                                                        @enderror
                                                    </label>
                                                </div>
                                            ...
                                            <div class="row">
                                                <div class="form-group col-md-12">
                                                    ...
                                                    <textarea
                                                        class="form-control summernote @error('in_app_notification_template.'.$value->id) is-invalid @enderror"
                                                        name="in_app_notification_template[{{ $value->id }}]">{{ old('in_app_notification_template.'.$value->id, $templates[$key]->in_app ?? '') }}</textarea>
                                                    @error('in_app_notification_template.'.$value->id)
                                                    <span class="invalid-feedback">
                                                        {{ __($message) }}
                                                    </span>
                                                    @enderror
